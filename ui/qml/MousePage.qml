...
Item {
    ...
    function layoutNoteText() {
        var _lang = lm.strings
        var noteKey = backend.deviceLayoutNoteKey || ""
        if (noteKey !== "" && s[noteKey])
            return s[noteKey]
        return backend.deviceLayoutNote || ""
    }

    function hotspotStatus(hotspot) {
        var status = String((hotspot && hotspot["status"]) || "mappable")
        return (status === "conditional" || status === "readonly")
               ? status : "mappable"
    }

    function hotspotStatusText(hotspot) {
        var _lang = lm.strings
        var status = hotspotStatus(hotspot)
        if (status === "mappable")
            return ""
        var key = String((hotspot && hotspot["statusTextKey"]) || "")
        if (key !== "" && s[key])
            return s[key]
        if (status === "conditional")
            return s["mouse.hotspot.conditional"] || "Needs raw HID detection"
        return s["mouse.hotspot.readonly"] || "Shown only"
    }

    function translatedControlList(controls) {
        var _lang = lm.strings
        var sep = s["common.list_separator"] || ", "
        var out = []
        for (var i = 0; i < controls.length; i++)
            out.push(lm.trButton(String(controls[i])))
        return out.join(sep)
    }
        ...
        ScrollView {
            ...
            Flickable {
                ...
                Column {
                    ...
                    Rectangle {
                        visible: backend.g502OnboardToolsVisible
                        width: parent.width - 56
                        height: visible ? g502OnboardCol.implicitHeight + 24 : 0
                        radius: 16
                        anchors.horizontalCenter: parent.horizontalCenter
                        color: Qt.rgba(0.83, 0.54, 0.00, uiState.darkMode ? 0.10 : 0.08)
                        border.width: 1
                        border.color: Qt.rgba(0.83, 0.54, 0.00, 0.35)
                        ...
                        Column {
                            ...
                            Text {
                                width: parent.width
                                text: s["mouse.g502_onboard_title"] || "G502 onboard button unlock"
                            ...
                            Text {
                                width: parent.width
                                text: s["mouse.g502_onboard_desc"]
                                      || "Back up the current onboard profile, then convert DPI/Profile buttons into F13-F16 keys Mouser can intercept."
                            ...
                            Text {
                                width: parent.width
                                text: (s["mouse.g502_onboard_last_backup"] || "Last backup: ")
                                      + backend.g502OnboardLastBackupPath
                                visible: backend.g502OnboardLastBackupPath !== ""
                            ...
                            Flow {
                                ...
                                Rectangle {
                                    width: g502BackupText.implicitWidth + 22
                                    height: 30
                                    radius: 8
                                    enabled: !backend.g502OnboardBusy
                                    opacity: enabled ? 1.0 : 0.55
                                    color: g502BackupMa.containsMouse
                                           ? Qt.rgba(1, 1, 1, 0.10)
                                           : Qt.rgba(1, 1, 1, 0.04)
                                    border.width: 1
                                    border.color: theme.border
                                    ...
                                    Text {
                                        id: g502BackupText
                                        anchors.centerIn: parent
                                        text: s["mouse.g502_onboard_backup"] || "Back up only"
                                        font { family: uiState.fontFamily; pixelSize: 11; bold: true }
                                        color: theme.textPrimary
                                    }
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            if (parent.enabled)
                                                backend.backupG502OnboardConfig()
                                        }
                                    }
                                }

                                Rectangle {
                                    width: g502UnlockText.implicitWidth + 22
                                    height: 30
                                    radius: 8
                                    enabled: !backend.g502OnboardBusy
                                    opacity: enabled ? 1.0 : 0.55
                                    color: g502UnlockMa.containsMouse
                                           ? theme.accent
                                           : Qt.rgba(1, 1, 1, 0.04)
                                    border.width: 1
                                    ...
                                    Text {
                                        id: g502UnlockText
                                        anchors.centerIn: parent
                                        text: s["mouse.g502_onboard_unlock"] || "Back up & unlock"
                                        font { family: uiState.fontFamily; pixelSize: 11; bold: true }
                                        color: g502UnlockMa.containsMouse ? "#101820" : theme.textPrimary
                                    }
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            if (parent.enabled)
                                                backend.unlockG502OnboardButtons()
                                        }
                                    }
                                }

                                Rectangle {
                                    width: g502RestoreText.implicitWidth + 22
                                    height: 30
                                    radius: 8
                                    enabled: !backend.g502OnboardBusy
                                             && backend.g502OnboardLastBackupPath !== ""
                                    opacity: enabled ? 1.0 : 0.45
                                    color: g502RestoreMa.containsMouse && enabled
                                           ? Qt.rgba(0.9, 0.3, 0.3, 0.18)
                                           : Qt.rgba(1, 1, 1, 0.04)
                                    border.width: 1
                                    border.color: Qt.rgba(0.9, 0.3, 0.3, 0.45)

                                    Text {
                                        id: g502RestoreText
                                        anchors.centerIn: parent
                                        text: s["mouse.g502_onboard_restore"] || "Restore latest backup"
                                        font { family: uiState.fontFamily; pixelSize: 11; bold: true }
                                        color: theme.textPrimary
                                    }
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            if (parent.enabled)
                                                backend.restoreG502OnboardBackup()
                                        }
                                    }
                                }
                            }
                    ...
                    Item {
                        ...
                        Repeater {
                            ...
                            delegate: HotspotDot {
                                ...
                                status: hotspotStatus(hotspot)
                                statusText: hotspotStatusText(hotspot)
                        ...
                        Rectangle {
                            visible: backend.mouseConnected
                                     && backend.hasInteractiveDeviceLayout
                                     && backend.deviceHotspots.length > 0
                            anchors {
                                left: parent.left
                                leftMargin: 28
                            ...
                            width: legendRow.implicitWidth + 24
                            height: 34
                            radius: 12
                            color: uiState.darkMode ? Qt.rgba(0, 0, 0, 0.32)
                                                    : Qt.rgba(1, 1, 1, 0.90)
                            border.width: 1
                            border.color: theme.border
                            ...
                            Row {
                                ...
                                Row {
                                    spacing: 6
                                    anchors.verticalCenter: parent.verticalCenter
                                    Rectangle { width: 9; height: 9; radius: 5; color: theme.accent }
                                    Text {
                                        text: s["mouse.legend_mappable"] || "Configurable"
                                        font { family: uiState.fontFamily; pixelSize: 10 }
                                        color: theme.textSecondary
                                    }
                                }
                                ...
                                Row {
                                    spacing: 6
                                    anchors.verticalCenter: parent.verticalCenter
                                    Rectangle { width: 9; height: 9; radius: 5; color: "#d48a00" }
                                    Text {
                                        text: s["mouse.legend_conditional"] || "Needs raw HID detection"
                                        font { family: uiState.fontFamily; pixelSize: 10 }
                                        color: theme.textSecondary
                                    }
                                }
                                ...
                                Row {
                                    spacing: 6
                                    anchors.verticalCenter: parent.verticalCenter
                                    Rectangle { width: 9; height: 9; radius: 5; color: Qt.rgba(0.52, 0.57, 0.64, 1) }
                                    Text {
                                        text: s["mouse.legend_readonly"] || "Shown only"
                                        font { family: uiState.fontFamily; pixelSize: 10 }
                                        color: theme.textSecondary
                                    }
                                }
                        ...
                        Rectangle {
                            ...
                            Column {
                                ...
                                Text {
                                    text: (s["mouse.physical_controls_prefix"] || "Physical / function controls: ")
                                          + translatedControlList(backend.devicePhysicalControls)
                                    width: parent.width
                                    wrapMode: Text.WordWrap
                                    font { family: uiState.fontFamily; pixelSize: 12 }
                                    color: theme.textSecondary
                                    visible: backend.devicePhysicalControls.length > 0
                                }

                                Text {
                                    text: s["mouse.mappable_controls"] || "Mouser can currently remap:"
                                    width: parent.width
                                    wrapMode: Text.WordWrap
                                ...
                                Repeater {
                                    ...
                                    delegate: Rectangle {
                                        ...
                                        Row {
                                            ...
                                            Text {
                                                text: { var _lang = lm.strings; return lm.trButton(modelData.name) }
                                            ...
                                            Text {
                                                text: { var _lang = lm.strings; return lm.trAction(modelData.actionLabel) }
                        ...
                        Column {
                                ...
                                Column {
                                    ...
                                    Text {
                                        text: {
                                            ...
                                            return selectedButtonName
                                                   ? lm.trButton(selectedButtonName) + (s["mouse.choose_action_suffix"] || " — Choose Action")
                                                   : ""
                                        }
                                        font { family: uiState.fontFamily; pixelSize: 15; bold: true }
                                        color: theme.textPrimary
                                    ...
                                    Text {
                                        text: selectedButton === "hscroll_left"
                                              ...
                                              : selectedHotspotStatus() === "conditional"
                                                ? s["mouse.conditional_button_action_hint"]
                            ...
                            Rectangle {
                                visible: selectedButton !== ""
                                         && selectedHotspotStatus() === "conditional"
                                width: parent.width
                                height: rawHidLearnCol.implicitHeight + 20
                                radius: 12
                                color: Qt.rgba(0.83, 0.54, 0.00, 0.08)
                                border.width: 1
                                border.color: Qt.rgba(0.83, 0.54, 0.00, 0.35)
                                ...
                                Column {
                                    ...
                                    Rectangle {
                                        width: rawHidStartText.implicitWidth + 24
                                        height: 30
                                        radius: 8
                                        color: rawHidStartMa.containsMouse
                                               ? theme.accent
                                               : Qt.rgba(1, 1, 1, 0.05)
                                        border.width: 1
                                        border.color: theme.accent
                                        ...
                                        Text {
                                            id: rawHidStartText
                                            anchors.centerIn: parent
                                            text: s["mouse.g502_onboard_unlock"]
                                                  || "Back up & unlock"
                                            font { family: uiState.fontFamily; pixelSize: 11; bold: true }
                                            color: rawHidStartMa.containsMouse
                                                   ? "#101820"
                                        ...
                                        MouseArea {
                                            ...
                                            hoverEnabled: true
                                            cursorShape: Qt.PointingHandCursor
                                            onClicked: backend.unlockG502OnboardButtons()
                                        }
                                    }
                                }
                            }
                    ...
                    Rectangle {
                        ...
                        Column {
                            ...
                            RowLayout {
                                ...
                                Rectangle {
                                    Layout.preferredWidth: debugRawHidText.implicitWidth + 20
                                    Layout.preferredHeight: 28
                                    radius: 8
                                    color: debugRawHidMa.containsMouse
                                           ? theme.accent
                                           : Qt.rgba(1, 1, 1, 0.04)
                                    border.width: 1
                                    border.color: debugRawHidMa.containsMouse
                                                  ? theme.accent
                                                  : Qt.rgba(0.83, 0.54, 0.00, 0.45)

                                    Text {
                                        id: debugRawHidText
                                        anchors.centerIn: parent
                                        text: s["mouse.start_g502_raw_hid"]
                                              || "Start G502 button detection"
                                        font { family: uiState.fontFamily; pixelSize: 11; bold: true }
                                        color: debugRawHidMa.containsMouse
                                               ? "#101820"
                                               : theme.textPrimary
                                    ...
                                    MouseArea {
                                        ...
                                        hoverEnabled: true
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: backend.startG502RawHidLearning()
                                    }
                                }
