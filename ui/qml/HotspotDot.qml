...
Item {
    ...
    property string status: "mappable"    // "mappable", "conditional", or "readonly"
    ...
    readonly property bool isReadonly: status === "readonly"
    readonly property bool isConditional: status === "conditional"
    readonly property color conditionalColor: "#d48a00"
    readonly property color readonlyColor: Qt.rgba(0.52, 0.57, 0.64, 1)
    readonly property color effectiveAccent: isReadonly ? readonlyColor
                                                        : isConditional ? conditionalColor
                                                                        : theme.accent
    readonly property color effectiveAccentDim: isReadonly ? Qt.rgba(0.52, 0.57, 0.64, 0.34)
                                                           : isConditional ? Qt.rgba(0.88, 0.54, 0.0, 0.38)
                                                                           : Qt.rgba(0, 0.83, 0.67, 0.3)
    property bool isSelected: !isReadonly && (isHScroll ? mousePage.selectedButton === "hscroll_left"
    ...
    Rectangle {
        ...
        color: isSelected && !isConditional ? theme.accentHover : effectiveAccent
        border.width: 2
        border.color: hotspot.activeFocus ? theme.textPrimary : Qt.rgba(0, 0, 0, 0.3)

        scale: isHovered && !isReadonly ? 1.2 : 1.0
    ...
    MouseArea {
        ...
        width: 36; height: 36
        hoverEnabled: true
        cursorShape: hotspot.isReadonly ? Qt.ArrowCursor : Qt.PointingHandCursor
    ...
    Canvas {
        ...
        onPaint: {
            ...
            ctx.strokeStyle = isSelected ? effectiveAccent : effectiveAccentDim
            ctx.lineWidth = 1
            ctx.setLineDash([4, 3])
            ctx.beginPath()
            ctx.moveTo(cx, cy)
        ...
        Connections {
            ...
            function onIsHoveredChanged() { lineCanvas.requestPaint() }
            function onStatusChanged() { lineCanvas.requestPaint() }
            function onEffectiveAccentChanged() { lineCanvas.requestPaint() }
            function onEffectiveAccentDimChanged() { lineCanvas.requestPaint() }
    ...
    Rectangle {
        ...
        color: isSelected
               ? (uiState.darkMode
                  ? Qt.rgba(effectiveAccent.r, effectiveAccent.g, effectiveAccent.b, 0.12)
                  : Qt.rgba(effectiveAccent.r, effectiveAccent.g, effectiveAccent.b, 0.16))
                          : uiState.darkMode ? Qt.rgba(0, 0, 0, 0.35) : Qt.rgba(1, 1, 1, 0.92)
        border.width: isSelected || hotspot.activeFocus || isConditional || isReadonly ? 1 : 0
        border.color: effectiveAccentDim
        ...
        Column {
            ...
            Text {
                ...
                color: isSelected ? effectiveAccent
                                  : isReadonly ? theme.textSecondary : theme.textPrimary
            }

            Text {
                text: hotspot.statusText
                font { family: uiState.fontFamily; pixelSize: 9; bold: hotspot.isConditional }
                color: effectiveAccent
                visible: hotspot.statusText !== ""
    ...
    Rectangle {
        ...
        color: isSelected ? effectiveAccent : effectiveAccentDim
    }
}
