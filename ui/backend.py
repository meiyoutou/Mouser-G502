...
from ui.locale_manager import translate_action_label, translate_button_label, translate_string
...
def _augment_with_button_gesture_keys(eff):
    ...
    for btn in SWIPE_CAPABLE_BUTTONS:
        if btn in eff_set:
            for key in (f"{btn}_tap", *swipe_direction_keys(btn)):
                if key not in eff_set:
                    eff_set.add(key)
                    ordered.append(key)
    return tuple(ordered)


def _ordered_known_keys(effective_keys, known_names):
    """Return known button keys in the device/layout order when one is known."""
    if effective_keys is None:
        return list(known_names.keys())
    result = []
    seen = set()
    for key in effective_keys:
        if key in known_names and key not in seen:
            result.append(key)
            seen.add(key)
    return result
...
class Backend(QObject):
            ...
            try:
                ...
                if self.startAtLogin:
                    ...
                    self._emit_status_key(
                        "status.start_login_enable_failed",
    ...
    def _language(self):
        lang = self._cfg.get("settings", {}).get("language", "en")
        return lang if lang in ("en", "zh_CN", "zh_TW") else "en"

    def _tr(self, key, default=None, **values):
        text = translate_string(self._language(), key, default)
        if values:
            try:
    ...
    def _tr_action_label(self, label):
        translated = translate_action_label(self._language(), label)
        if translated != label:
            return translated
        return translate_button_label(self._language(), label)
    ...
    def _tr_g502_onboard_error(self, message):
        ...
        backup_match = re.match(r"(.+)\. Backup was saved at (.+)$", text)
        if backup_match:
            return self._tr(
                "error.g502_onboard.backup_saved_after_error",
                "{error}. Backup was saved at {path}",
                error=self._tr_g502_onboard_error(backup_match.group(1)),
                path=backup_match.group(2),
            )

        exact = {
            "G502 onboard profile descriptor is too short":
                "error.g502_onboard.descriptor_short",
            "Negative sector is invalid":
                "error.g502_onboard.negative_sector",
            "G502 sector length must be a positive multiple of 16":
                "error.g502_onboard.invalid_sector_length",
            "Could not open G502 onboard profile interface":
                "error.g502_onboard.open_interface_failed",
            "Could not determine the active G502 onboard profile":
                "error.g502_onboard.active_profile_unknown",
            "G502 onboard profile directory CRC is invalid; refusing to continue":
                "error.g502_onboard.directory_crc_invalid",
            "No G502 onboard profile headers were found":
                "error.g502_onboard.no_profile_headers",
            "G502 onboard write verification failed; backup was saved but the profile did not read back as expected":
                "error.g502_onboard.write_verify_failed",
            "No G502 onboard backup file was found":
                "error.g502_onboard.no_backup_file",
            "Selected file is not a Mouser G502 onboard backup":
                "error.g502_onboard.invalid_backup_file",
            "Backup contains no G502 onboard sectors":
                "error.g502_onboard.backup_no_sectors",
            "Backup has an invalid G502 sector size":
                "error.g502_onboard.backup_invalid_sector_size",
            "Connected G502 sector size does not match the backup":
                "error.g502_onboard.connected_sector_size_mismatch",
        }
        key = exact.get(text)
        if key:
            return self._tr(key, text)

        patterns = [
            (
                r"Unsupported G502 memory model (0x[0-9A-Fa-f]+)$",
                "error.g502_onboard.unsupported_memory_model",
                "Unsupported G502 memory model {value}",
                ("value",),
            ),
            (
                r"Unsupported G502 profile format (0x[0-9A-Fa-f]+)$",
                "error.g502_onboard.unsupported_profile_format",
                "Unsupported G502 profile format {value}",
                ("value",),
            ),
            (
                r"Unsupported G502 macro format (0x[0-9A-Fa-f]+)$",
                "error.g502_onboard.unsupported_macro_format",
                "Unsupported G502 macro format {value}",
                ("value",),
            ),
            (
                r"Unexpected G502 button count (\d+)$",
                "error.g502_onboard.unexpected_button_count",
                "Unexpected G502 button count {count}",
                ("count",),
            ),
            (
                r"Unexpected G502 sector count (\d+)$",
                "error.g502_onboard.unexpected_sector_count",
                "Unexpected G502 sector count {count}",
                ("count",),
            ),
            (
                r"Unexpected G502 sector size (\d+)$",
                "error.g502_onboard.unexpected_sector_size",
                "Unexpected G502 sector size {size}",
                ("size",),
            ),
            (
                r"HID backend is not available: (.+)$",
                "error.g502_onboard.hid_backend_unavailable",
                "HID backend is not available: {error}",
                ("error",),
            ),
            (
                r"No supported G502 HERO HID\+\+ interface was found \(expected ([^)]+)\)$",
                "error.g502_onboard.no_supported_interface",
                "No supported G502 HERO HID++ interface was found (expected {device})",
                ("device",),
            ),
            (
                r"Could not open G502 onboard profile interface: (.+)$",
                "error.g502_onboard.open_interface_failed_detail",
                "Could not open G502 onboard profile interface: {error}",
                ("error",),
            ),
            (
                r"HID\+\+ request timed out for feature=(0x[0-9A-Fa-f]+) function=([^ ]+) params=\[(.*)\]$",
                "error.g502_onboard.hidpp_timeout",
                "HID++ request timed out for feature={feature} function={function} params=[{params}]",
                ("feature", "function", "params"),
            ),
            (
                r"Active G502 profile sector (0x[0-9A-Fa-f]+) was not backed up$",
                "error.g502_onboard.active_sector_not_backed_up",
                "Active G502 profile sector {sector} was not backed up",
                ("sector",),
            ),
            (
                r"Active G502 profile sector (0x[0-9A-Fa-f]+) has an invalid CRC$",
                "error.g502_onboard.active_sector_crc_invalid",
                "Active G502 profile sector {sector} has an invalid CRC",
                ("sector",),
            ),
            (
                r"Backup sector key (.+) is invalid$",
                "error.g502_onboard.backup_sector_key_invalid",
                "Backup sector key {sector} is invalid",
                ("sector",),
            ),
            (
                r"Backup sector (0x[0-9A-Fa-f]+) is invalid$",
                "error.g502_onboard.backup_sector_invalid",
                "Backup sector {sector} is invalid",
                ("sector",),
            ),
            (
                r"Backup sector (0x[0-9A-Fa-f]+) contains invalid hex data$",
                "error.g502_onboard.backup_sector_invalid_hex",
                "Backup sector {sector} contains invalid hex data",
                ("sector",),
            ),
            (
                r"Backup sector (0x[0-9A-Fa-f]+) has invalid length$",
                "error.g502_onboard.backup_sector_invalid_length",
                "Backup sector {sector} has invalid length",
                ("sector",),
            ),
            (
                r"Backup sector (0x[0-9A-Fa-f]+) CRC is invalid; refusing to restore$",
                "error.g502_onboard.backup_sector_crc_invalid",
                "Backup sector {sector} CRC is invalid; refusing to restore",
                ("sector",),
            ),
            (
                r"Backup sector (0x[0-9A-Fa-f]+) is outside the connected G502 memory$",
                "error.g502_onboard.backup_sector_outside_memory",
                "Backup sector {sector} is outside the connected G502 memory",
                ("sector",),
            ),
        ]
        for regex, key, default, names in patterns:
            match = re.match(regex, text)
            if match:
                values = dict(zip(names, match.groups()))
                return self._tr(key, default, **values)
        return text

    def _tr_debug_text(self, message):
        text = str(message or "")
        if not text:
            return text
        exact = {
            "Debug mode enabled": "debug.mode_enabled",
            "Debug mode disabled": "debug.mode_disabled",
            "Debug event capture enabled": "debug.events_enabled",
            "Debug event capture paused": "debug.events_paused",
            "Gesture recording enabled": "debug.gesture_recording_enabled",
            "Gesture recording disabled": "debug.gesture_recording_disabled",
            "G502 Raw HID detection is only available on Windows Raw Input.":
                "debug.g502_raw_windows_only",
            "G502 onboard buttons already emit F13-F16 or no supported special actions were found.":
                "debug.g502_onboard_already_unlocked",
            "G502 onboard profile unlocked. DPI/Profile buttons now emit F13-F16 for Mouser to intercept.":
                "debug.g502_onboard_profile_unlocked",
            "G502 onboard backup restored.":
                "debug.g502_onboard_backup_restored",
            "Mouse connected": "debug.mouse_connected",
            "Mouse disconnected": "debug.mouse_disconnected",
        }
        key = exact.get(text)
        if key:
            return self._tr(key, text)
        match = re.match(r"G502 Raw HID detection failed: (.+)$", text)
        if match:
            return self._tr(
                "debug.g502_raw_failed_detail",
                "G502 Raw HID detection failed: {error}",
                error=match.group(1),
            )
        match = re.match(r"G502 onboard backup: (.+)$", text)
        if match:
            return self._tr(
                "debug.g502_onboard_backup_path",
                "G502 onboard backup: {path}",
                path=match.group(1),
            )
        match = re.match(r"G502 onboard unlock: (.+) -> (.+)$", text)
        if match:
            return self._tr(
                "debug.g502_onboard_unlock_mapping",
                "G502 onboard unlock: {label} -> {target_key}",
                label=self._tr_action_label(match.group(1)),
                target_key=match.group(2),
            )
        match = re.match(r"G502 onboard operation failed: (.+)$", text)
        if match:
            return self._tr(
                "debug.g502_onboard_failed_detail",
                "G502 onboard operation failed: {error}",
                error=self._tr_g502_onboard_error(match.group(1)),
            )
        return text

    def _emit_status_text(self, message):
        text = str(message or "")
        if not text:
            return
        exact = {
            "Saved": "status.saved",
            "Profile already exists": "status.profile_exists",
            "Profile created": "status.profile_created",
            "Profile deleted": "status.profile_deleted",
            "Connect a device first": "status.connect_device_first",
            "Unknown layout option": "status.unknown_layout",
            "Experimental layout applied": "status.layout_applied",
            "Layout reset to auto-detect": "status.layout_auto",
            "G502 Raw HID detection unavailable": "status.g502_raw_unavailable",
            "G502 Raw HID detection failed": "status.g502_raw_failed",
            "G502 Raw HID detection started": "status.g502_raw_started",
            "G502 onboard operation is already running": "status.g502_onboard_busy",
            "G502 onboard backup saved": "status.g502_onboard_backup_saved",
            "G502 onboard buttons unlocked": "status.g502_onboard_unlocked",
            "G502 onboard backup restored": "status.g502_onboard_restored",
            "Update check already running": "status.update_check_running",
            "Checking for updates...": "status.checking_updates",
            "Mouser is up to date": "status.up_to_date",
            "Could not check for updates": "status.update_check_failed",
            "Update installed": "status.update_installed",
            "Update is already in progress": "status.update_in_progress",
            "Open the release page to install manually": "status.update_manual",
            "Update is not ready to install": "status.update_not_ready",
            "Choose a valid screenshot folder": "status.choose_screenshot_folder",
            "Start at login is not available on this platform": "status.start_login_unavailable",
            "Start at login could not be enabled. Please try again.": "status.start_login_enable_failed",
        ...
        if text.startswith("Profile: "):
            ...
            return
        match = re.match(r"Mouser (.+) is available$", text)
        if match:
            self._emit_status_key(
                "status.update_available",
                "Mouser {version} is available",
                version=match.group(1),
            )
            return
        match = re.match(r"Updated to (.+)$", text)
        if match:
            self._emit_status_key(
                "status.updated_to",
                "Updated to {version}",
    ...
    def _compute_buttons(self):
        ...
        layout_button_labels = self._device_layout.get("button_labels", {})
        result = []
        idx = 0
        for key in _ordered_known_keys(self._effective_supported_buttons, BUTTON_NAMES):
            name = BUTTON_NAMES[key]
            aid = mappings.get(key, "none")
            idx += 1
            result.append({
                "key": key,
                "name": layout_button_labels.get(key, name),
                "actionId": aid,
    ...
    @Property(str, notify=deviceLayoutChanged)
    def deviceLayoutNoteKey(self):
        return self._device_layout.get("note_key", "")

    @Property(list, notify=deviceLayoutChanged)
    def devicePhysicalControls(self):
        return list(self._device_layout.get("physical_controls", []))
    ...
    @Property(bool, notify=deviceLayoutChanged)
    def g502OnboardToolsVisible(self):
        return (
            sys.platform.startswith("win")
            and self._device_layout.get("key") == "g502"
        )

    @Property(bool, notify=g502OnboardChanged)
    def g502OnboardBusy(self):
        return bool(self._g502_onboard_busy)

    @Property(str, notify=g502OnboardChanged)
    def g502OnboardLastBackupPath(self):
        settings = self._cfg.get("settings", {})
        pre_unlock = str(settings.get("g502_onboard_pre_unlock_backup", "") or "")
        if pre_unlock and g502_onboard.backup_path_has_locked_special_buttons(pre_unlock):
            return pre_unlock
        saved = str(
            settings.get("g502_onboard_last_backup", "")
            or ""
        )
        if saved and g502_onboard.backup_path_has_locked_special_buttons(saved):
            return saved
        latest = (
            g502_onboard.latest_before_unlock_backup_path()
            or g502_onboard.latest_backup_path()
        )
        return str(latest or "")

    @Property(bool, notify=g502OnboardChanged)
    def g502OnboardUnlocked(self):
        return bool(
            self._cfg.get("settings", {}).get("g502_onboard_unlocked", False)
        )
    ...
    def _startUpdateCheck(self, manual=False):
        ...
        if self._update_check_in_progress:
            if manual:
                self._emit_status_key("status.update_check_running", "Update check already running")
            return
        self._update_check_in_progress = True
        if manual:
            self._emit_status_key("status.checking_updates", "Checking for updates...")
    ...
    def _handleUpdateCheckFinished(self, manual, reachable, state_data):
        ...
        if manual:
            if reachable:
                self._emit_status_key("status.up_to_date", "Mouser is up to date")
            else:
                self._emit_status_key("status.update_check_failed", "Could not check for updates")
        ...
        try:
            ...
            if status == "installed":
                ...
                if version:
                    ...
                    self._emit_status_key("status.update_installed", "Update installed")
            elif status == "failed":
    ...
    def chooseScreenshotDirectory(self):
        ...
        selected = QFileDialog.getExistingDirectory(
            None,
            self._tr("dialog.choose_screenshot_folder", "Choose Screenshot Folder"),
            current,
        )
        ...
        if not os.path.isdir(normalized):
            self._emit_status_key(
                "status.choose_screenshot_folder",
    ...
    def prepareLatestUpdate(self):
        if self.updateInstallInProgress:
            self._emit_status_key("status.update_in_progress", "Update is already in progress")
            return
    ...
    def installPreparedUpdate(self):
        if not self.updateInstallEnabled:
            self._emit_status_key("status.update_manual", "Open the release page to install manually")
            return
        if not self._pending_update_plan_path or not self._update_install_can_install:
            self._emit_status_key("status.update_not_ready", "Update is not ready to install")
            return
    ...
    def setStartAtLogin(self, value):
        ...
        try:
            ...
            self._emit_status_key(
                ...
                "Failed to update login item: {error}",
                error=str(exc),
            )
            return
        ...
        try:
            ...
            if rollback_error is not None:
                ...
                self._emit_status_key(
                    ...
                    "Failed to save login item setting: {error}",
                    error=str(exc),
                )
            return
        self.settingsChanged.emit()
    ...
    def setLanguage(self, lang):
        normalized = lang if lang in {"en", "zh_CN", "zh_TW"} else "en"
        settings = self._cfg.setdefault("settings", {})
        if settings.get("language", "en") == normalized:
            return
    ...
    @Slot()
    def startG502RawHidLearning(self):
        """Open a short read-only capture window for G502 vendor HID reports."""
        if not self.debugMode:
            self.setDebugMode(True)
        elif not self._debug_events_enabled:
            self.setDebugEventsEnabled(True)

        if not self._engine or not hasattr(self._engine, "start_raw_hid_learning"):
            self._append_debug_line(
                "G502 Raw HID detection is only available on Windows Raw Input."
            )
            self._emit_status_key(
                "status.g502_raw_unavailable",
                "G502 Raw HID detection unavailable",
            )
            return

        try:
            started = bool(self._engine.start_raw_hid_learning(60))
        except Exception as exc:
            self._append_debug_line(f"G502 Raw HID detection failed: {exc}")
            self._emit_status_key(
                "status.g502_raw_failed",
                "G502 Raw HID detection failed",
            )
            return

        if started:
            self._emit_status_key(
                "status.g502_raw_started",
                "G502 Raw HID detection started",
            )
        else:
            self._append_debug_line(
                "G502 Raw HID detection is only available on Windows Raw Input."
    ...
    def _run_g502_onboard_operation(self, operation, success_message, *args):
        if self._g502_onboard_busy:
            self._emit_status_key(
                "status.g502_onboard_busy",
                "G502 onboard operation is already running",
            )
            return {}
        self._g502_onboard_busy = True
        self.g502OnboardChanged.emit()
        try:
            if self._engine and hasattr(self._engine, operation):
                result = getattr(self._engine, operation)(*args)
            else:
                fallback = {
                    "backup_g502_onboard_config": g502_onboard.backup_g502_hero,
                    "unlock_g502_onboard_buttons": g502_onboard.unlock_g502_hero_buttons,
                    "restore_g502_onboard_backup": g502_onboard.restore_g502_hero_backup,
                }[operation]
            ...
            if backup_path:
                settings = self._cfg.setdefault("settings", {})
                if (
                    operation == "unlock_g502_onboard_buttons"
                    and result.get("changed") is False
                    and settings.get("g502_onboard_pre_unlock_backup")
                ):
                    pass
                else:
                    settings["g502_onboard_last_backup"] = backup_path
                if (
                    operation == "unlock_g502_onboard_buttons"
                    and result.get("changed") is True
                ):
                    settings["g502_onboard_pre_unlock_backup"] = backup_path
                save_config(self._cfg)
            self._append_debug_line(result.get("message", success_message))
            if backup_path:
                self._append_debug_line(f"G502 onboard backup: {backup_path}")
            for change in result.get("changes", []) or []:
                label = change.get("label", change.get("button_key", "?"))
                to_info = change.get("to", {}) or {}
                target_key = (
                    change.get("target_key")
                    or to_info.get("key_name")
                    or to_info.get("raw_hex", "?")
                )
                self._append_debug_line(
                    f"G502 onboard unlock: {label} -> {target_key}"
                )
            self._emit_status_text(success_message)
            return result
        except Exception as exc:  # noqa: BLE001 - surface to UI without crashing
            error_text = self._tr_g502_onboard_error(str(exc))
            self._append_debug_line(f"G502 onboard operation failed: {error_text}")
            self._emit_status_key(
    ...
    def backupG502OnboardConfig(self):
        """Back up the connected G502 HERO onboard profile sectors."""
        return self._run_g502_onboard_operation(
            "backup_g502_onboard_config",
            "G502 onboard backup saved",
        )

    @Slot(result="QVariantMap")
    def unlockG502OnboardButtons(self):
        """Back up then make G502 DPI/profile buttons visible to Mouser."""
        result = self._run_g502_onboard_operation(
            "unlock_g502_onboard_buttons",
            "G502 onboard buttons unlocked",
        )
        if result.get("ok"):
            self._cfg.setdefault("settings", {})["g502_onboard_unlocked"] = True
            save_config(self._cfg)
            if self._engine:
                self._engine.cfg = self._cfg
                self._engine.reload_mappings()
    ...
    @Slot(result="QVariantMap")
    def restoreG502OnboardBackup(self):
        """Restore the most recent G502 HERO onboard profile backup."""
        result = self._run_g502_onboard_operation(
            "restore_g502_onboard_backup",
            "G502 onboard backup restored",
            self.g502OnboardLastBackupPath or None,
        )
        if result.get("ok"):
            self._cfg.setdefault("settings", {})["g502_onboard_unlocked"] = False
            save_config(self._cfg)
            if self._engine:
                self._engine.cfg = self._cfg
                self._engine.reload_mappings()
    ...
    def addProfile(self, appId):
        ...
        if self._profile_has_app(app_spec):
            self._emit_status_key("status.profile_exists", "Profile already exists")
            return
        ...
        if self._engine:
            self._engine.cfg = self._cfg
        self.profilesChanged.emit()
        self._emit_status_key("status.profile_created", "Profile created")

    @Slot()
    def browseForAppProfile(self):
        """Open a file picker, then create a profile for the selected app."""
        from PySide6.QtWidgets import QFileDialog
        title = self._tr("dialog.select_application", "Select Application")
        if sys.platform == "darwin":
            path, _ = QFileDialog.getOpenFileName(
                None, title, "/Applications", "Apps (*.app)")
    ...
    def deleteProfile(self, name):
        ...
        if self._engine:
            ...
            self._engine.reload_mappings()
        self.profilesChanged.emit()
        self._emit_status_key("status.profile_deleted", "Profile deleted")
    ...
    def getProfileMappings(self, profileName):
        ...
        layout_button_labels = self._device_layout.get("button_labels", {})
        result = []
        for key in _ordered_known_keys(self._effective_supported_buttons, PROFILE_BUTTON_NAMES):
            name = PROFILE_BUTTON_NAMES[key]
            aid = mappings.get(key, "none")
            result.append({
                "key": key,
                "name": layout_button_labels.get(key, name),
    ...
    def setDeviceLayoutOverride(self, layoutKey):
        ...
        if not self._mouse_connected or not device_key:
            self._emit_status_key("status.connect_device_first", "Connect a device first")
            return
        valid_choices = {choice["key"] for choice in get_manual_layout_choices()}
        if normalized not in valid_choices:
            self._emit_status_key("status.unknown_layout", "Unknown layout option")
            return
        ...
        if normalized:
            self._emit_status_key("status.layout_applied", "Experimental layout applied")
        else:
            self._emit_status_key("status.layout_auto", "Layout reset to auto-detect")
    ...
    def _onEngineShowRing(self, slots, interactive=False):
        ...
        labels = [
            _resolve_ring_label(
                s,
                self._tr_action_label(ACTIONS.get(s, {}).get("label", s)),
                language,
            )
            for s in slots
        ]
    ...
    def _handleStatusMessage(self, message):
        """Runs on Qt main thread."""
        self._emit_status_text(message)

    def _append_debug_line(self, message):
        timestamp = time.strftime("%H:%M:%S")
        self._debug_lines.append(f"[{timestamp}] {self._tr_debug_text(message)}")
