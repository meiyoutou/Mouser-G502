...
class Engine:
    ...
    def _setup_hooks(self, *, defer_wheel_invert=False):
        ...
        if hasattr(self.hook, "g502_onboard_unlocked"):
            self.hook.g502_onboard_unlocked = bool(
                settings.get("g502_onboard_unlocked", False)
            )
        ring_btn_key = next(
        ...
        device_button_set = set(device_buttons) if device_buttons is not None else None
        ...
        for btn_key, action_id in mappings.items():
            if not isinstance(action_id, str):
                continue
            # Do not wire mappings for buttons the connected device/layout does
            # not advertise. This matters for G502 Raw Input extra-button
            # events: the same raw bit can mean a different control on another
            # Logitech mouse, so a saved G502 profile must not fire elsewhere.
            if (
                device_button_set is not None
                and btn_key in BUTTON_TO_EVENTS
                and btn_key not in device_button_set
            ):
                continue
    ...
    def start_raw_hid_learning(self, duration_s=60):
        """Start a short, read-only Raw HID learning window when supported."""
        if not self._debug_events_enabled:
            self.set_debug_events_enabled(True)
        starter = getattr(self.hook, "start_raw_hid_learning", None)
        if not starter:
            return False
        return bool(starter(duration_s))
    ...
    def _run_with_hid_listener_paused(self, func):
        """Run an operation that needs exclusive access to Logitech HID++.

        The background HidGestureListener may keep the same vendor HID
        interface open.  Pause it around one-shot onboard-profile operations so
        backup/write/restore requests do not race with the live listener.
        """
        hook = self.hook
        listener_running = getattr(hook, "_hid_gesture", None) is not None
        if listener_running and hasattr(hook, "_stop_hid_listener"):
            hook._stop_hid_listener()
            time.sleep(0.1)
        try:
            return func()
        finally:
            if listener_running and hasattr(hook, "_start_hid_listener"):
                hook._start_hid_listener()

    def backup_g502_onboard_config(self):
        """Export the connected G502 HERO onboard profile sectors."""
        from core.g502_onboard import backup_g502_hero

        return self._run_with_hid_listener_paused(backup_g502_hero)

    def unlock_g502_onboard_buttons(self):
        """Back up then remap G502 HERO DPI/profile controls to F13-F16."""
        from core.g502_onboard import unlock_g502_hero_buttons

        return self._run_with_hid_listener_paused(unlock_g502_hero_buttons)

    def restore_g502_onboard_backup(self, backup_path=None):
        """Restore a G502 HERO onboard profile backup."""
        from core.g502_onboard import restore_g502_hero_backup

        return self._run_with_hid_listener_paused(
            lambda: restore_g502_hero_backup(backup_path)
        )
