...
from core.logi_device_catalog import G502_OS_LEVEL_BUTTONS
from core.updater import UpdateCheckState
from ui.locale_manager import translate_string
...
class BackendDeviceLayoutTests(unittest.TestCase):
    ...
    def test_start_g502_raw_hid_learning_enables_debug_and_calls_engine(self):
        engine = _FakeEngine()
        backend = self._make_backend(engine=engine)

        backend.startG502RawHidLearning()

        self.assertTrue(backend.debugMode)
        self.assertTrue(engine.debug_enabled)
        self.assertEqual(engine.raw_hid_learning_requests, [60])

    def test_selected_language_localizes_status_and_debug_messages(self):
        cfg = copy.deepcopy(DEFAULT_CONFIG)
        cfg.setdefault("settings", {})["language"] = "en"
        ...
        with patch("ui.backend.save_config") as save_config_mock:
            backend.setLanguage("zh_CN")
        save_config_mock.assert_called_once_with(backend._cfg)
        self.assertEqual(backend._cfg["settings"]["language"], "zh_CN")

        backend._emit_status_text("G502 onboard buttons unlocked")
        backend._append_debug_line("Mouse connected")
        backend._append_debug_line(r"G502 onboard backup: C:\backup.json")
        backend._append_debug_line("G502 onboard unlock: DPI down -> F14")
        backend._append_debug_line(
            "G502 onboard operation failed: No G502 onboard backup file was found"
        )

        self.assertEqual(
            statuses,
            [translate_string("zh_CN", "status.g502_onboard_unlocked")],
        )
        self.assertIn("鼠标已连接", backend.debugLog)
    ...
    def test_g502_layout_exposes_extra_configurable_buttons(self):
        device = SimpleNamespace(
            key="g502_hero",
        ...
        self.assertTrue(backend.hasInteractiveDeviceLayout)
        self.assertEqual(backend.effectiveDeviceLayoutKey, "g502")
        self.assertEqual(
            [button["key"] for button in backend.buttons],
            [
                "middle",
                "xbutton1",
                "xbutton2",
        ...
        self.assertEqual(
            [button["name"] for button in backend.buttons],
            [
                "Wheel click",
                "Back thumb button",
        ...
        hotspots = {hotspot["buttonKey"]: hotspot for hotspot in backend.deviceHotspots}
        self.assertEqual(hotspots["g502_dpi_shift"]["status"], "conditional")
        self.assertEqual(hotspots["g502_dpi_down"]["status"], "conditional")
        self.assertEqual(hotspots["g502_dpi_up"]["status"], "conditional")
        self.assertEqual(hotspots["g502_profile_cycle"]["status"], "conditional")
        self.assertEqual(hotspots["g502_left_click"]["status"], "readonly")
        self.assertEqual(hotspots["g502_wheel_mode_shift"]["status"], "readonly")
