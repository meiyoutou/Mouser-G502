...
class UpdateInstallerTests(unittest.TestCase):
    ...
    def test_process_runner_hides_windows_run_console_window(self):
        completed = subprocess.CompletedProcess(["helper"], 0, "", "")
        with (
            patch("core.update_installer.sys.platform", "win32"),
            ...
            patch("core.update_installer.subprocess.run", return_value=completed) as run,
        ):
            self.assertIs(ProcessRunner().run(["helper"]), completed)
    ...
    def test_process_runner_hides_windows_popen_console_window(self):
        proc = object()
        with (
            patch("core.update_installer.sys.platform", "win32"),
            patch.object(subprocess, "CREATE_NO_WINDOW", 0x08000000, create=True),
            ...
            patch.object(subprocess, "SW_HIDE", 0, create=True),
            patch.object(subprocess, "STARTUPINFO", _FakeStartupInfo, create=True),
            patch("core.update_installer.subprocess.Popen", return_value=proc) as popen,
        ):
            self.assertIs(ProcessRunner().popen(["helper"]), proc)
