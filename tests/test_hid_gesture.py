...
class HidDiscoveryDiagnosticsTests(unittest.TestCase):
    ...
    def test_try_connect_accepts_g502_hero_without_reprog_controls(self):
        listener = hid_gesture.HidGestureListener()
        info = {
            "product_id": 0xC08B,
            "usage_page": 0xFF00,
            ...
            "transport": "USB",
            "source": "hidapi-enumerate",
            "product_string": "G502 HERO Gaming Mouse",
            "path": b"/dev/hidraw-g502-hero",
        }
        fake_dev = _FakeHidDevice()
        ...
        with (
            ...
            patch("core.hid_gesture._save_last_device_cache") as save_cache_mock,
            patch("builtins.print"),
        ):
            self.assertTrue(listener._try_connect())

        device = listener.connected_device
        self.assertEqual(device.key, "g502_hero")
        self.assertEqual(device.gesture_cids, ())
        self.assertIn("xbutton1", device.supported_buttons)
        self.assertIn("xbutton2", device.supported_buttons)
        self.assertIn("hscroll_left", device.supported_buttons)
        self.assertIn("hscroll_right", device.supported_buttons)
        diagnostic_codes = {
            diagnostic.code for diagnostic in device.capability_inventory.diagnostics
        }
        self.assertIn("os_level_only_device", diagnostic_codes)
