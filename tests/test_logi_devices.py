...
class LogiDeviceRegistryTests(unittest.TestCase):
    ...
    def test_resolve_g502_hero_by_product_id(self):
        ...
        self.assertFalse(device.requires_reprog_controls)
        self.assertEqual(device.gesture_cids, ())

    def test_resolve_g502_hero_by_logitech_alias(self):
        device = resolve_device(product_name="Logitech G502 Hero")
    ...
    def test_resolve_g502_x_plus_by_alias(self):
        ...
        self.assertEqual(device.ui_layout, "g502_x")
        self.assertFalse(device.requires_reprog_controls)
    ...
    def test_g502_buttons_include_host_visible_extra_controls(self):
    ...
    def test_g502_x_buttons_are_os_level_only(self):
        self.assertEqual(get_buttons_for_layout("g502_x"), get_buttons_for_layout("g502"))

    def test_g502_layout_is_interactive_not_generic(self):
        layout = get_device_layout("g502")

        self.assertEqual(layout["key"], "g502")
        self.assertTrue(layout["interactive"])
        self.assertEqual(layout["image_asset"], "logitech-mice/g502/mouse.svg")
        self.assertGreater(len(layout["hotspots"]), 0)
        self.assertIn("11 G HUB-programmable buttons", layout["note"])
        self.assertIn("DPI Shift/sniper", layout["note"])
        self.assertIn("wheel-mode shift", layout["note"])
        self.assertEqual(layout["note_key"], "layout_note.g502_11")
        self.assertEqual(layout["button_labels"]["middle"], "Wheel click")
        self.assertEqual(layout["button_labels"]["g502_dpi_shift"], "DPI Shift / sniper")
        self.assertEqual(layout["button_labels"]["g502_dpi_down"], "DPI down")
        self.assertEqual(layout["button_labels"]["g502_dpi_up"], "DPI up")
        self.assertEqual(layout["button_labels"]["g502_profile_cycle"], "Profile cycle")
        self.assertEqual(layout["button_labels"]["hscroll_left"], "Wheel tilt left")
        self.assertIn("DPI Shift / sniper", layout["physical_controls"])
        self.assertIn("Wheel mode shift (mechanical)", layout["physical_controls"])
        ...
        for key in (
            ...
            self.assertEqual(hotspots[key]["status"], "conditional")
        for key in (
            "g502_left_click",
            "g502_right_click",
            "g502_wheel_mode_shift",
        ):
            self.assertEqual(hotspots[key]["status"], "readonly")

        g502_x_layout = get_device_layout("g502_x")

        self.assertEqual(g502_x_layout["key"], "g502_x")
        self.assertTrue(g502_x_layout["interactive"])
        self.assertEqual(g502_x_layout["image_asset"], "logitech-mice/g502_x/mouse.svg")
        self.assertGreater(len(g502_x_layout["hotspots"]), 0)
        self.assertIn("13 G HUB-programmable controls", g502_x_layout["note"])
        self.assertIn("adjustable DPI-Shift", g502_x_layout["note"])
        self.assertEqual(g502_x_layout["note_key"], "layout_note.g502_x")
        self.assertIn("Scroll up", g502_x_layout["physical_controls"])
        self.assertIn("Scroll down", g502_x_layout["physical_controls"])
        g502_x_hotspots = {
            hotspot["buttonKey"]: hotspot for hotspot in g502_x_layout["hotspots"]
        }
        self.assertEqual(g502_x_hotspots["g502_scroll_up"]["status"], "readonly")
        self.assertEqual(g502_x_hotspots["g502_scroll_down"]["status"], "readonly")
    ...
    def test_build_g502_hero_connected_info(self):
        ...
        self.assertIn("g502_dpi_shift", info.supported_buttons)
        self.assertIn("g502_dpi_down", info.supported_buttons)
        self.assertIn("g502_dpi_up", info.supported_buttons)
        self.assertIn("g502_profile_cycle", info.supported_buttons)
