...
class DeviceLayoutTests(unittest.TestCase):
        ...
        for device in KNOWN_LOGI_DEVICES:
            ...
            for hotspot in layout["hotspots"]:
                with self.subTest(device=device.key, button=hotspot["buttonKey"]):
                    if hotspot.get("status") == "readonly":
                        continue
    ...
    def test_manual_choices_include_gaming_family_layouts(self):
        ...
        self.assertIn({"key": "g502", "label": "G502 / HERO / LIGHTSPEED"}, choices)
        self.assertIn({"key": "g502_x", "label": "G502 X / X LIGHTSPEED / X PLUS"}, choices)

    def test_g502_layouts_are_interactive_with_statused_hotspots(self):
        ...
        for layout_key, asset in (
            ("g502", "logitech-mice/g502/mouse.svg"),
            ("g502_x", "logitech-mice/g502_x/mouse.svg"),
        ):
            with self.subTest(layout=layout_key):
                ...
                self.assertTrue(layout["interactive"])
                self.assertEqual(layout["image_asset"], asset)
                self.assertTrue(expected_configurable.issubset(hotspots))

                for key in expected_configurable:
                    status = hotspots[key].get("status", "mappable")
                    expected = "conditional" if key in expected_conditional else "mappable"
                    self.assertEqual(status, expected)

                self.assertEqual(hotspots["g502_left_click"]["status"], "readonly")
                self.assertEqual(hotspots["g502_right_click"]["status"], "readonly")
                self.assertEqual(hotspots["g502_wheel_mode_shift"]["status"], "readonly")

    def test_g502_x_shows_scroll_directions_as_readonly(self):
        layout = get_device_layout("g502_x")
        hotspots = {hotspot["buttonKey"]: hotspot for hotspot in layout["hotspots"]}

        self.assertEqual(hotspots["g502_scroll_up"]["status"], "readonly")
        self.assertEqual(hotspots["g502_scroll_down"]["status"], "readonly")
